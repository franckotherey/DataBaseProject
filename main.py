...
conn = psycopg2.connect (
    ...
    host = "",
    port = "",
    options = "-c search_path=poryecto1k"
)
...
def generate_stock(n):
    ...
    while i < n:
        try:
            fk = random.choicer(res1)
            direccion = random.choice(res2)[0]
            cantidad = random.randint(0, 999)
            cursor.execute("SELECT nombre_producto, medida, direccion FROM stock WHERE nombre_producto=''{fk[0]}' AND medida='{fk[1]}' AND direccion='{direcciion}'")
...
def generate_compra_local(n):
    ...
    while i < n:
        try:
            ...
            if not cursor.fetchone():
                cursor.execute(f"INSERT INTO compralocal(correlativom numero, direccion) VALUES ('{correlativo}', '{fk[0]}', '{fk[1]}');")
